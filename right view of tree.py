...
class Tree:
    ...
    def rightview(self,val):
        ...
        return res
    #using recursion
    def rightview2(self,val,d,level):
        if val!=None:
            if level not in d:
                d[level]=val.data
            self.rightview2(val.right,d,level+1)
            self.rightview2(val.left,d,level+1)
        res=[]
        for i in sorted(d.keys()):
            res.append(d[i])
        return res
